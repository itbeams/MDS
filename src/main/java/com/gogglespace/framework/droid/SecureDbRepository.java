...
/**
 *  - Droid Persistence Framework DB repository <br /> 
 *  - A generic repository for cipher db operations <br />
 *  - Added support for connection pooling
 * @author Shahid Nawaz on 2/4/2015.
 * @version 1.0, 1.1
 */
public class SecureDbRepository<T> extends BaseMTSecureDBRepository<T> {

    public SecureDbRepository(Context context, T o) {
        super(context, o);
    }
	...
	public <T> T get(long id) {
        ...
        DbRepositoryHelper.EntityDetails entityDetails =  helper.getEntityDetailsOnly(type);
        SQLiteDatabase db = getDb(context);
        Cursor cursor = db.query(entityDetails.tableName, entityDetails.selectedColumnList,
        		QueryUtils.getWhereClauseIdField(entityDetails.getIDField()), QueryUtils.getSelectionArg(id), null, null, null);
        EntityInstanceFactory instanceFactory = new EntityInstanceFactory();
        Map<String, Field> entityFieldCache = null;
        // now fetch items and iterate over
        T entity = null;
	...
	public <T> T saveOrUpdate(T obj) {
        ...
        if (!update) {
        	...
        	db.update(tableName, cv, QueryUtils.getWhereClauseIdField(idField), QueryUtils.getSelectionArg(rowId));
    	...
    	if (objList != null && !objList.isEmpty()) {
    		SQLiteDatabase db = getDb(context);
	    		for(T obj : objList) {
	        		...
	        		fieldList = AnnotationHelper.getFieldsInfo(obj,
	                        AnnotationHelper.FIELD_INFO.INCLUDE_VALUES);
	                for (FieldInfo f : fieldList) {
	                    if (tableName == null) {
	                        tableName = f.getTableName();
	...
	public <T> int delete(T obj){
        ...
        boolean execStatus = db.delete(entityDetails.getTableName(), 
        				QueryUtils.getWhereClauseIdField(entityDetails.getIDField()), 
        				QueryUtils.getSelectionArg(baseEntity.getId())) > 0;
        if (execStatus) {
            status = 1;
        }
        close();
	...
	public void resetSequence(T obj, long sequence) {
        ...
        if (hasSequence) {
        	...
        	cv.put("NAME", entityDetails.tableName);
